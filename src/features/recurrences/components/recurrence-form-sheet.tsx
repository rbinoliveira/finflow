...
function defaultValues(recurrence: Recurrence | null): RecurrenceSchema {
  if (recurrence) {
    return {
...
export function RecurrenceFormSheet({
  open,
  recurrence,
  onClose,
  onRemove,
}: RecurrenceFormSheetProps) {
  ...
  const {
    ...
    defaultValues: defaultValues(recurrence),
  })

  useEffect(() => {
    if (open) reset(defaultValues(recurrence))
